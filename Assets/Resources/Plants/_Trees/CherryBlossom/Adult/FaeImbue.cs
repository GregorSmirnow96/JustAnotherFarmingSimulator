...
public class FaeImbue : MonoBehaviour, IInteractable
{
    ...
    public void Interact()
    {
        ...
        bool equippedItemIsImbuable = imbuableItems.ItemIdIsFaeImbuable(equippedItemId);

        if (equippedItemIsImbuable)
        {
            string imbuedId = imbuableItems.GetFaeImbuedItemId(equippedItem.type.id);
            if (imbuedId != null)
            {
