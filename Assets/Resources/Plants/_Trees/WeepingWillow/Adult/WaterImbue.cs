...
public class WaterImbue : MonoBehaviour, IInteractable
{
    ...
    public void Interact()
    {
        ...
        bool equippedItemIsImbuable = imbuableItems.ItemIdIsWaterImbuable(equippedItemId);

        if (equippedItemIsImbuable)
        {
            string imbuedId = imbuableItems.GetWaterImbuedItemId(equippedItem.type.id);
            if (imbuedId != null)
            {
