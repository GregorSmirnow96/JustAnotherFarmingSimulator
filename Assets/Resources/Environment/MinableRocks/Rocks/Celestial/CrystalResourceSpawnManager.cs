...
public class CrystalResourceSpawnManager : MonoBehaviour
{
    public int maxFilledContainers = 2;
    ...
    private List<ResourceContainer> GetEmptyContainers() => resourceContainers
        ...
        .Where(container => 
            {
                Debug.Log((container.transform.position - SceneProperties.playerTransform.position).magnitude);
                return (container.transform.position - SceneProperties.playerTransform.position).magnitude >= minSpawnDistanceFromPlayer;
            })
        .ToList();
}
